...
- 複数行のコメントも可能
- COMMENT_CONTENT は字句解析されず生のまま読み飛ばされ、最初に出現する `]}` で終端する
- `]}` の直前の `-` は内容ではなく右 trim フラグとして解釈される（内容は `-` で終われない）
- `]}` が現れる前に入力が終了した場合は構文エラー（閉じられていないコメント）
- 空白制御（trim）との併用が可能: `{[-% ... ]}`, `{[% ... -]}`, `{[-% ... -]}`
- コメント自体は出力を生成しないが、trim によって周囲の空白・改行を制御できる
