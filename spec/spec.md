...
Natsuzoraは、HTML生成に特化したミニマルなテンプレート言語である。
...
- デフォルトで安全: 明示的な指定がある場合を除き、すべての変数値はHTMLエスケープされる。
...
- 空白(Whitespace): スペース (`U+0020`)、タブ (`U+0009`)、改行 (LF `U+000A`, CR `U+000D`)。
- 識別子(Identifier): `[A-Za-z][A-Za-z0-9_]*` にマッチする文字列のうち、予約語以外のもの。`_`で始まる識別子は予約されており使用できない。
- パス(Path): `Identifier` をドット `.` で連結したもの（例: `author.name`）。
- インクルード名(Include Name): `/`で始まり、`Identifier`をスラッシュ`/`で連結した論理名（例: `/components/card`）。
- タグ(Tag): `{[` で始まり `]}` で終わる構文要素。
...
設計意図: 浮動小数点について、JavaScriptでは`3`と`3.0`の区別をつけないため。
...
設計意図: NatsuzoraではルートのJSONオブジェクトは表示のために渡される情報であるため、StringまたはIntergerだけあれば十分である。
Null、Boolean、Array、Mapは誤って渡されたものとみなす。`null`や`false`を渡された場合に`""`や`"false"`といった文字列と解釈することも考えられるが、そうしたいのであればJSONオブジェクトを生成・加工するタイミングで`""`や`"false"`にすればよい。
...
- コメント内容は字句解析されず生のまま読み飛ばされる。`%` 以降、最初に出現する `]}`で終端し、それまでの任意の文字（記号、改行、`{[` を含む）を許容する。
- `]}` の直前の文字が `-` の場合、その `-` は常に右 trim フラグとして解釈され、内容には含まれない。したがってコメント内容は文字 `-` で終わることができない（`{[%-]}` は空内容＋右 trim のコメント）。
- `]}` が現れる前に入力が終了した場合は構文エラー（閉じられていないコメント）とする。
...
正例/誤例:
- 正: `{[% comment ]}`, `{[-% c -]}`, `{[% <div class="x">, {y: [1]}. ]}`
- 誤: `{[ % comment ]}`（`{[`と`%`の間に空白）、`{[% comment`（閉じられていない）
