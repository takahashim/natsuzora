...
`natsuzora-contract` CLI は、テンプレートおよびコントラクトをプロジェクト内の通常ファイルとして扱う。
意図しないプロジェクト外ファイルの読み書きを避けるため、以下を安全上の制約とする。

- include の解決結果が `include_root` 外を指す場合はエラーとする。シンボリックリンクによる `include_root` 外への参照もエラーとする。
- テンプレートおよびコントラクトのディレクトリ走査では、シンボリックリンクのディレクトリを辿らない。
- `sync` / `apply` が `.ntzc` を書き込むとき、対象ファイルまたは親ディレクトリの経路にシンボリックリンクが含まれる場合はエラーとする。
- これらの制約は、信頼できないリポジトリや作業ツリーに対して CLI を実行した場合のファイル漏えい・意図しない上書きを防ぐためのものである。
